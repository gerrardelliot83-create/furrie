...
interface CarePlanDetailContentProps {
  planId: string;
}

export function CarePlanDetailContent({ planId }: CarePlanDetailContentProps) {
  ...
  return (
    <div className={styles.container}>
      ...
      {plan.pets && (
        <div className={styles.section}>
          <div className={styles.sectionTitle}>Pet</div>
          <div className={styles.petRow}>
            ...
            <div className={styles.petInfo}>
              <span className={styles.petName}>{plan.pets.name}</span>
              {plan.pets.breed && (
                <span className={styles.petBreed}>
                  {plan.pets.breed} ({plan.pets.species})
                </span>
              )}
            </div>
          </div>
      ...
      {plan.vet && (
        <div className={styles.section}>
          <div className={styles.sectionTitle}>Veterinarian</div>
          <div className={styles.vetRow}>
            {plan.vet.avatar_url ? (
              ...
              <div className={styles.vetAvatar}>
                <span className={styles.vetFallback}>
                  {plan.vet.full_name.charAt(0)}
                </span>
              </div>
            )}
            <span className={styles.vetName}>Dr. {plan.vet.full_name}</span>
          </div>
        </div>
      )}
      ...
      <div className={styles.section}>
        ...
        <div className={styles.stepsList}>
          {plan.care_plan_steps?.map((step) => {
            ...
            const isPastDue =
              step.due_date && !isCompleted && new Date(step.due_date) < new Date();
                  ...
                  <div className={styles.stepRow}>
                    ...
                    <div className={styles.stepContent}>
                      <div className={styles.stepHeader}>
                        <h3
                          className={`${styles.stepTitle} ${isCompleted ? styles.stepTitleCompleted : ''}`}
                        >
                          {step.title}
                        </h3>
                      </div>
                      ...
                      <div className={styles.stepMeta}>
                        ...
                        {step.due_date && (
                          <span
                            className={`${styles.dueDate} ${isPastDue ? styles.dueDateOverdue : ''}`}
                          >
                            Due:{' '}
                      ...
                      {step.care_plan_step_responses &&
                        step.care_plan_step_responses.length > 0 && (
                                      ...
                                      if (mediaType === 'video') {
                                        return (
                                          <video
                                            ...
                                            controls
                                            preload="metadata"
                                            className={styles.responseMediaVideo}
                                          />
                                        );
                                      }
                                      return (
                      ...
                      {!isCompleted && (
                        <div className={styles.stepActions}>
                          {isVideoCheckIn ? (
                            <Link href={`/connect?petId=${plan.pet_id}`}>
                            ...
                            isExpanded ? (
                              <StepResponseForm
                                onSubmit={(text, urls, types) =>
                                  handleResponseSubmit(step.id, text, urls, types)
                                }
